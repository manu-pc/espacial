...
public class DAOLibros extends AbstractDAO {
    ...
    public Libro consultarLibro(Integer idLibro){
        ...
        if (rsLibro.next())
        {
            ...
            try {
            stmEjemplares = con.prepareStatement(
                "SELECT e.num_ejemplar, e.ano_compra, e.localizador, " +
                "       p.usuario, p.fecha_prestamo " +
                "FROM ejemplar e " +
                "LEFT JOIN prestamo p ON e.libro = p.libro " +
                "                   AND e.num_ejemplar = p.ejemplar " +
            ...
            {
            resultado.addEjemplar(new Ejemplar(
                    resultado,
                rsEjemplares.getInt("num_ejemplar"),
                rsEjemplares.getString("localizador"),
                rsEjemplares.getString("ano_compra"),
                rsEjemplares.getString("usuario"),
                rsEjemplares.getDate("fecha_prestamo") != null 
                    ? rsEjemplares.getDate("fecha_prestamo").toLocalDate() 
                    : null
            ));
            }
            } catch (SQLException e){
...
public java.util.List<Ejemplar> consultarEjemplaresLibro(Integer idLibro) {
    java.util.List<Ejemplar> resultado = new java.util.ArrayList<>();
    Connection con;
    PreparedStatement stmEjemplares = null;
    ResultSet rsEjemplares;
    ...
    try {
        ...
        try {
            if (stmEjemplares != null) stmEjemplares.close();
        } catch (SQLException e) {
...
public void borrarEjemplaresLibro(Integer idLibro, java.util.List<Integer> numsEjemplar) {
    Connection con;
    PreparedStatement stmCheckPrestamosActivos = null;
    PreparedStatement stmDeletePrestamos = null;
    PreparedStatement stmDeleteEjemplar = null;
    ResultSet rs = null;
    
    con = super.getConexion();
    
    try {
        // Consulta para verificar si hay préstamos activos
        String checkPrestamosSQL = "SELECT COUNT(*) FROM prestamo WHERE libro = ? AND ejemplar = ? AND fecha_devolucion IS NULL";
        stmCheckPrestamosActivos = con.prepareStatement(checkPrestamosSQL);
        
        // Consulta para eliminar préstamos NO activos (fecha_devolucion != NULL)
        String deletePrestamosSQL = "DELETE FROM prestamo WHERE libro = ? AND ejemplar = ? AND fecha_devolucion IS NOT NULL";
        stmDeletePrestamos = con.prepareStatement(deletePrestamosSQL);
        
        // Consulta para eliminar el ejemplar
        String deleteEjemplarSQL = "DELETE FROM ejemplar WHERE libro = ? AND num_ejemplar = ?";
        stmDeleteEjemplar = con.prepareStatement(deleteEjemplarSQL);
        
        for (Integer i : numsEjemplar) {
            // Verificar si hay préstamos activos
            stmCheckPrestamosActivos.setInt(1, idLibro);
            stmCheckPrestamosActivos.setInt(2, i);
            rs = stmCheckPrestamosActivos.executeQuery();
            
            if (rs.next() && rs.getInt(1) > 0) {
                // Si hay préstamos activos, no eliminamos nada
                this.getFachadaAplicacion().muestraExcepcion("No se puede eliminar el ejemplar porque tiene préstamos acitvos.");
                continue;
            }
            
            // Eliminar préstamos NO activos
            stmDeletePrestamos.setInt(1, idLibro);
            stmDeletePrestamos.setInt(2, i);
            stmDeletePrestamos.executeUpdate();
            
            // Intentar eliminar el ejemplar
            stmDeleteEjemplar.setInt(1, idLibro);
            stmDeleteEjemplar.setInt(2, i);
            stmDeleteEjemplar.executeUpdate();
        }
    } catch (SQLException e) {
        System.out.println(e.getMessage());
        this.getFachadaAplicacion().muestraExcepcion(e.getMessage());
    } finally {
        try { if (rs != null) rs.close(); } catch (SQLException e) { System.out.println("Imposible cerrar ResultSet"); }
        try { if (stmCheckPrestamosActivos != null) stmCheckPrestamosActivos.close(); } catch (SQLException e) { System.out.println("Imposible cerrar cursor de verificación"); }
        try { if (stmDeletePrestamos != null) stmDeletePrestamos.close(); } catch (SQLException e) { System.out.println("Imposible cerrar cursor de eliminación de préstamos"); }
        try { if (stmDeleteEjemplar != null) stmDeleteEjemplar.close(); } catch (SQLException e) { System.out.println("Imposible cerrar cursor de eliminación de ejemplares"); }
    }
}
